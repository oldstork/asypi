...
namespace Asypi {
    ...
    */
    ...
    class Worker {
        ...
        /// </summary>
        public Task Run(HttpListener listener) {
            return Task.Run(async () => {
                Stopwatch stopwatch = new Stopwatch();
                
                // restart after previous request handled
                while (true) {
                    // block until listener context acquired
                    // (request sent and delegated to this worker)
                    HttpListenerContext context = await listener.GetContextAsync();
                    
                    // reset stopwatch once listener context acquired
                    stopwatch.Reset();
                    stopwatch.Start();
